...
function extractB3dm(b3dmBuffer) {
    if (!b3dmBuffer) {
        throw new Error('b3dmBuffer is not defined.');
    }
    var magic = getMagic(b3dmBuffer);
    if (magic !== 'b3dm') {
        throw new Error('Invalid magic, expected "b3dm", got: "' + magic + '".');
    }
    var version = b3dmBuffer.readUInt32LE(4);
    if (version !== 1) {
        throw new Error('Invalid version, only "1" is valid, got: "' + version + '".');
    }
